...
GPT-5.2 PDF dosyalarını **doğrudan PDF formatında** işleyebilir (hem metin hem görseller).

**Nasıl Çalışır?**
- PDF'den hem metin hem de her sayfanın görüntüsü çıkarılır
- Model her iki veriyi de kullanarak analiz yapar
- Diagramlar, tablolar, görseller de analiz edilebilir

**En İyi Yöntem:** Responses API (`/v1/responses`)
    ...
    input=[
        {
            ...
            "content": [
                {
                    "type": "input_text",
                    "text": "Bu PDF'in içeriğini özetle ve önemli tabloları çıkar."
                },
                {
                    "type": "input_file",  # PDF için input_file
...
response2 = client.responses.create(
    model="gpt-5.2",
    input="Şu PDF'i analiz et: https://example.com/dokuman.pdf"
)
# Model otomatik olarak PDF'i indirecek ve analiz edecek
```
...
with open("dokuman.pdf", "rb") as file:
    uploaded_file = client.files.create(
        file=file,
        purpose="user_data"  # PDF'ler için user_data veya assistants
    )

print(f"Dosya yüklendi: {uploaded_file.id}")
    ...
    input=[
        {
            ...
            "content": [
                {
                    "type": "input_text",
                    "text": "Bu PDF'teki ana konuları listele ve tabloları özetle."
                },
                {
                    "type": "input_file",  # PDF için input_file kullan
                    "file_id": uploaded_file.id
                }
            ]
        }
...
# Alternatif: Daha basit format (tek string input)
response2 = client.responses.create(
    model="gpt-5.2",
    input=[
        {
...
### Yöntem 3: Chat Completions API ile PDF
...
# Chat Completions API, PDF'yi doğrudan destekler
# Ancak Responses API kullanımı önerilir (daha güçlü)
...
response = client.chat.completions.create(
    ...
    messages=[
        {
            "role": "user",
            "content": "Bu PDF'teki tabloları çıkar ve analiz et."
        }
    ]
    # Not: Chat Completions'da PDF desteği sınırlıdır
    # PDF işleme için Responses API (Yöntem 1-2) önerilir
)
...
**⚠️ ÖNEMLİ NOT:** 
PDF işleme için **Responses API kullanımı şiddetle önerilir**. Chat Completions API'de PDF desteği sınırlıdır veya farklı çalışabilir.
...
## Ek A: PDF İşleme Detayları

### PDF'ler Nasıl İşlenir?

OpenAI API'si PDF'leri şu şekilde işler:

1. **Metin Çıkarma:** PDF'teki tüm metin içeriği otomatik çıkarılır
2. **Görsel Çıkarma:** Her sayfa bir görüntü olarak da işlenir
3. **Dual Input:** Model hem metni hem görselleri birlikte analiz eder

Bu sayede:
- Diagramlardaki bilgiler okunabilir
- Tablolar analiz edilebilir
- Görsel + metin birlikte yorumlanır

### Hangi API PDF Destekler?

| API | PDF Desteği | Yöntem |
|-----|-------------|--------|
| **Responses API** | ✅ Tam destek | `input_file` ile file_id veya URL |
| **Chat Completions API** | ⚠️ Sınırlı | Responses API önerilir |
